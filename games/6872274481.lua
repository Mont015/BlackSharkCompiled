...
run(function()
	...
	bedwars = setmetatable({
		...
		DefaultKillEffect = {new = function() return {onKill = function() end, isPlayDefaultKillEffect = function() return false end} end},
...
run(function()
	...
	local function hasGround(root, direction, extraDistance)
		rayParams.FilterDescendantsInstances = {lplr.Character, gameCamera}
		local checkPosition = root.Position + (direction * (extraDistance or 6))
		return workspace:Raycast(checkPosition, Vector3.new(0, -18, 0), rayParams) ~= nil
	end

	local function getSafestDirection(root, sourcePosition)
		...
		if not VoidCheck.Enabled then
			return directions[1]
		end

		for _, dir in directions do
			if hasGround(root, dir) then
				return dir
			end
		end
	...
	local function getEnemyPredictedDirection(attackerRoot)
		if not attackerRoot then return nil end
		local vel = attackerRoot.AssemblyLinearVelocity * Vector3.new(1, 0, 1)
		if vel.Magnitude < 1 then return nil end
		return vel.Unit
	end

	local function getDodgeDirection(root, sourcePosition, attackerCharacter)
		local attackerRoot = getRoot(attackerCharacter)
		local enemyMoveDir = getEnemyPredictedDirection(attackerRoot)

		if PredictFollowup.Enabled and enemyMoveDir then
			local perpLeft = Vector3.new(-enemyMoveDir.Z, 0, enemyMoveDir.X)
			local perpRight = -perpLeft

			if not VoidCheck.Enabled then
				strafeSide = -strafeSide
				return strafeSide == 1 and perpLeft or perpRight
			end

			if hasGround(root, perpLeft) then return perpLeft end
			if hasGround(root, perpRight) then return perpRight end
		end

		return getSafestDirection(root, sourcePosition)
	end
	...
	local function handleDamage(damageTable)
		...
		local now = tick()
		if now - lastHitTime < 1.5 and damageTable.fromEntity == lastAttacker then
			comboCount = math.min(comboCount + 1, 5)
		else
			comboCount = 1
		end
		lastHitTime = now
		lastAttacker = damageTable.fromEntity

		if now < nextDodge then return end

		local direction = getDodgeDirection(root, sourcePosition, damageTable.fromEntity)
		if not direction then return end

		local comboMultiplier = ComboBoost.Enabled and (1 + (comboCount - 1) * 0.3) or 1
		local dodgeDuration = Duration.Value * comboMultiplier

		dodgeDirection = direction
		dodgeUntil = now + dodgeDuration
		nextDodge = now + Cooldown.Value
	end

	local function scheduleRedodge(root)
		if not PredictFollowup.Enabled then return end

		task.delay(Duration.Value + 0.05, function()
			if not AntiHit.Enabled or not entitylib.isAlive or not entitylib.character then return end
			if not lastAttacker or not lastAttacker.Parent then return end
			if tick() - lastHitTime > 2 then return end

			local attackerRoot = getRoot(lastAttacker)
			if not attackerRoot then return end

			local dist = (root.Position - attackerRoot.Position).Magnitude
			if dist > TriggerRange.Value then return end

			local now = tick()
			if now >= nextDodge then
				local dir = getDodgeDirection(root, attackerRoot.Position, lastAttacker)
				if dir then
					dodgeDirection = dir
					dodgeUntil = now + Duration.Value
					nextDodge = now + Cooldown.Value
				end
			end
		end)
	...
	AntiHit = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				AntiHit:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
					local root = entitylib.character and entitylib.character.RootPart
					handleDamage(damageTable)
					if root then scheduleRedodge(root) end
				end))
				...
				AntiHit:Clean(runService.PreSimulation:Connect(function()
					...
					local comboMultiplier = ComboBoost.Enabled and (1 + (comboCount - 1) * 0.25) or 1
					...
					root.AssemblyLinearVelocity = Vector3.new(
						dodgeDirection.X * speed,
						vertical,
						dodgeDirection.Z * speed
					)
				end))
			else
				dodgeDirection = nil
	...
	ComboBoost = AntiHit:CreateToggle({
		Name = 'Combo boost',
		Default = true,
		Tooltip = 'Dodges faster and further the more times you get hit in a row.'
	})
	PredictFollowup = AntiHit:CreateToggle({
		Name = 'Predict followup',
		Default = true,
...
run(function()
	...
	local LongJumpMethods = {
		cannon = function(_, pos, dir)
			if not entitylib.isAlive or not entitylib.character or not entitylib.character.RootPart or not pos or not dir then return end
		...
		jade_hammer = function(item, _, dir)
			if not item or not dir then return end
		...
		tnt = function(item, pos, dir)
			if not item or not pos or not dir or not start then return end
			if not entitylib.isAlive or not entitylib.character or not entitylib.character.RootPart then return end
		...
		wood_dao = function(item, pos, dir)
			if not item or not pos or not dir or not lplr.Character then return end
			...
			if (lplr.Character:GetAttribute('CanDashNext') or 0) > workspace:GetServerTimeNow() or not bedwars.AbilityController:canUseAbility('dash') then
				repeat
					task.wait()
					if not LongJump.Enabled or not lplr.Character then return end
			...
			if LongJump.Enabled and lplr.Character then
				bedwars.SwordController.lastAttack = workspace:GetServerTimeNow()
		...
		Function = function(callback)
			...
			if callback then
				...
				LongJump:Clean(vapeEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
					if not dataTable or dataTable.hookFunction ~= 'PLAYER_IN_TRANSIT' then return end
					if not entitylib.isAlive or not entitylib.character or not entitylib.character.RootPart then return end
				...
				LongJump:Clean(runService.PreSimulation:Connect(function(dt)
					...
					if root and isnetworkowner(root) then
						if JumpTick > tick() then
							...
							if entitylib.character.Humanoid and entitylib.character.Humanoid.FloorMaterial == Enum.Material.Air and not start then
				...
				if store.hand and store.hand.tool and LongJumpMethods[store.hand.tool.Name] then
					...
					task.spawn(LongJumpMethods[store.hand.tool.Name], getItem(store.hand.tool.Name), start, (CameraDir.Enabled and gameCamera or root).CFrame.LookVector)
					return
				end
				...
				for i, v in LongJumpMethods do
					...
					if item or store.equippedKit == i then
						...
						task.spawn(v, item, start, (CameraDir.Enabled and gameCamera or root).CFrame.LookVector)
						break
					end
				end
			else
